...
private static final long serialVersionUID=411443363707927999L;
	...
	public EigenDecomposition eig(ComplexMatrix matrix)
	{
	...
	double[] evalues;
		...
		org.apache.commons.math3.linear.EigenDecomposition eig;
			...
			eig=new org.apache.commons.math3.linear.EigenDecomposition(aug);
			...
			for(int i=0;i<aug.getColumnDimension();i++) evectors.add(eig.getEigenvector(i).toArray());
		}
...
nextev:		for(int column=0;column<delta;column++)
		...
		double avgr,avgi;
			...
			{
				ev=new ComplexVector(evalues.length);//an eigenvector as a column vector
				...
				for(int row=0;row<ev.size();row++)
				{
					...
					ev.setValue(row,avgr,avgi);
				}
				result.add(evalues[column],ev);
			}
			return result;
		}
	}
	...
	EigenDecomposition result;
		...
		cov=new ComplexMatrix(real,imag);
		...
		System.out.println(Arrays.toString(result.eigenvalues()));
		...
		System.out.println(BLAS.multiply(cov,result.eigenvectors(),null));
