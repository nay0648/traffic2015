...
/**
 * Detect lanes by the Parzen window estimation.
 * @author nay0648
 */
public class ParzenLaneDetector extends LaneDetector
{
private static final long serialVersionUID = -6000323245680943584L;
...
private double[] gaussian=gaussian(SIGMA);//the Gaussian function
...
private int dfradius=2;//detector filter radius
	 ...
	 * number of lanes
	 */
	public ParzenLaneDetector(SliceGenerator slicer,int numlanes)
	{
	...
	int radius,i1,i2;
		...
		else
		{
			...
			if(posdata[posidx]>=0) 
			{	
				i1=posdata[posidx]-radius;
				if(i1<0) i1=0;
				i2=posdata[posidx]+radius;
				if(i2>laneres.length-1) i2=laneres.length-1;	
				
				for(int i=i1;i<=i2;i++) 
					laneres[i]-=gaussian[i-posdata[posidx]+radius];
			}
